...
class ModelHandler:
    ...
    # def encodeColumn(self, encode_dict):
    #     self.x_train = self.x_train.replace(encode_dict).infer_objects(copy=False)
    #     self.x_test = self.x_test.replace(encode_dict).infer_objects(copy=False)
...
model_handler.save_model_and_encoder('xgb_model.pkl', 'onehot_encoder.pkl')
